...
SNS·검색·기사·경쟁사 프로모션 신호를 Notion CMS와 GitHub Actions 배치로 수집·정리하는 온스타일 프로모션 트렌드 AI 기획 도구의 React 기반 Phase 1 MVP입니다.
...
- 주간/최근 기간 선택: `public/data/weeks.json` 기반 최신 완료주 기본값, 최근 7/14/30일, 최근 4/8주 합산 선택
...
- 주간 트렌드 요약 카드
- 기간 선택 패널: 기본값은 `주간 > 최신 완료주`, 필요 시 최근 7일 / 최근 14일 / 최근 30일 / 최신 완료주·직전 주·최근 4/8주 합산 드롭다운으로 전환
...
현재 버전은 Notion CMS에서 Published 상태로 정리된 실제 수집 데이터를 정적 JSON으로 변환해 표시합니다. 실제 운영 확대 전에는 출처 검증, 플랫폼 약관, 사내 보안 기준 검토가 필요합니다.
...
- 자동/수동 GitHub Actions:
  - `.github/workflows/daily-trend-update.yml`: 매일 07:00 KST 수집 → 자동 큐레이션 → Notion sync → Pages 배포까지 한 번에 실행
  - `.github/workflows/collect-trend-signals.yml`: 필요 시 수동으로 외부 신호 수집만 실행
  - `.github/workflows/sync-notion-data.yml`: 필요 시 수동으로 Published Notion 데이터를 정적 JSON으로 동기화하고 Pages까지 배포
...
뉴스 RSS + 홈쇼핑형/대형 이커머스 경쟁사 공개 프로모션 페이지를 Evidence Items DB에 Status=Draft로 upsert
  ↓
curate:trends가 노이즈/중복/테스트 근거는 Archived, 대표 근거는 Published로 정리하고 Trend Topic + Promotion Ideas를 갱신
  ↓
sync:notion이 Published Notion 데이터를 public/data/trends/latest.json으로 변환
  ↓
테스트/빌드 후 public/data 변경분 commit
  ↓
같은 workflow 안에서 GitHub Pages artifact 업로드 + 배포
```
...
- 기본 상태값은 `Draft`입니다. 일일 자동 운영에서는 `curate:trends`가 대표성 있는 근거를 `Published`로 전환하고, 노이즈/중복/테스트 근거는 `Archived`로 정리합니다. 수동 운영 시에는 Notion에서 직접 검토 후 `Published`로 변경할 수 있습니다.
...
3. 일일 자동 workflow(`daily-trend-update.yml`)의 source error율, Published/Archived 결과, Pages 배포 로그를 주기 점검
4. 로그인 전 개인 찜 UX 검증
